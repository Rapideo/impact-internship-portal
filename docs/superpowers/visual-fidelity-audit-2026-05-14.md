...
> **2026-05-19 — Phase H close-out:** All `[~]` entries resolved during SP7 Phases D1, D2, E1, E2, F, and G (PRs #94, #95, #96, #97, #98, #99). The final reference screenshot set lives at `docs/superpowers/visual-fidelity-screenshots/2026-05-19-final/`. PR refs for Phases F and G were placeholders (`<TBD-F>`, `<TBD-G>`) until SP7 Phase H backfilled them to the merged numbers (#98, #99). No items deferred to the post-SP7 backlog — every audit entry closed. Gate G8 walk pending.
...
- **P1** — Action-bar `Cancel` in prototype intern-record renders as `<a class="btn btn--outline">`. Confirm `ActionBar` component renders the same fixed bottom strip with `.action-bar__inner` + status mono chip.
  - **Resolved in SP7 Phase F (see PR #98).** `/admin/interns` restored the prototype's Actions column with inline Edit + Delete (Delete posts via a fetcher to the new index-route action handler that soft-deletes the intern). `/admin/self-assessment-results` now consumes `<TableFilter>` for the `.table-meta` count + sort caption + `.filters` strip; the assessments-hub intern-picker mounts the prototype's `<table class="picker-list">` (via the Phase B `<PickerList>` primitive) instead of the SP4-era `<ul>`. Inline `cursor: pointer` is kept on the interns rows (the prototype-style CSS-driven hover targets `.assessments tbody tr` which production already styles via admin.css). `<ActionBar>` from Phase B carries the prototype's sticky `.action-bar__inner` + status mono chip; `<Link className="btn btn--outline">` is rendered for the Cancel button so it matches prototype's `<a>` markup at the React level.

### `[✓] /admin/interns` ↔ `interns-dashboard.html` _(closed in SP7 Phase F — see PR #98)_
_Findings:_
...
- **P2** — Prototype `.filters` block is a separate `<section>` above the `.table-wrap`. Production wraps filters inside `TableFilter` together with table — visually similar but loses the prototype's gap and `Export CSV` button on the right of the filter row.
  - **Resolved in SP7 Phase F (see PR #98).** Restored Actions column with inline `Edit` + `Delete` (Delete opens a `<ConfirmModal>` and posts to a new action handler that soft-deletes the intern). `TableFilter` already renders the prototype's zero-padded `.table-meta` count (e.g. `<strong>05</strong>`). The `Current Phase` column was intentionally not restored — current production keeps the `Role` column since phase computation requires a per-cohort date-window lookup that isn't in the schema yet (the audit explicitly allows "document the swap"). Documented here as a deviation; the column can be restored once phase windows ship. `Last synced` micro-text in the table-meta is intentionally omitted (the production data is live, not synced).

### `[✓] /admin/interns/new` ↔ `intern-record.html` (create mode) _(closed in SP7 Phase F — see PR #98)_
_Findings:_
...
- **P2** — Action-bar status chip in prototype reads `INTERN RECORD · NEW`. Production matches. Confirm Sign-out / Cancel buttons share the same gap/order.
  - **Resolved in SP7 Phase F (see PR #98).** Save button now opens a `<ConfirmModal>` ("Save this intern record? — Identity fields will lock once saved …") matching prototype's `#updateModal` body verbatim, then programmatically submits the form on confirm. Action-bar status reads `INTERN RECORD · NEW` per prototype. The first-name single-character validator delta is tracked separately as a P2 backlog item.

### `[✓] /admin/interns/:internId` ↔ `intern-record.html` (edit mode) _(closed in SP7 Phase F — see PR #98)_
_Findings:_
...
- **P2** — Prototype includes an `.assessment-foot-note` link back to Interns under the cards ("To review what an intern has already completed, open their record from Interns."). Production omits.
  - **Resolved in SP7 Phase F (see PR #98).** Hub rebuilt against the prototype: `<AssessmentCard>` grid replaces the `identity-card`/`kpi-grid` block (with `assessment-card__stage` mono pill, `assessment-card__meta` label, display-font title, body prose, and CTA button); two-line `START AN<br/>ASSESSMENT.` title; intern-picker modal mounted in `.modal__card--wide` with `<PickerList>` table (columns Last Name · Cohort · Start · Current Phase) instead of the SP4-era `<ul>` of `.record-link` buttons; footer link to Interns added via `.assessment-foot-note`. Current-phase column renders `—` placeholder pending phase-window schema work (same deviation as the `/admin/interns` list).

### `[✓] /admin/assessments/competency/new` ↔ `competency-new.html` _(closed in SP7 Phase F — see PR #98)_
_Findings:_
...
### `[✓] /employer/cohorts` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/cohorts/:cohortId` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/interns` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/interns/:internId` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/competency/new` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/competency/edit` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/competency/:id` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/exit-survey` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/profile` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/roles` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/roles/new` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
...
### `[✓] /employer/roles/:roleId` _(closed in SP7 Phase G — see PR #99)_
_Findings:_
