...
The frozen prototype (`Rapideo/impact-prototype`) keeps the IMPACT gold branding — it is a reference, not restyled. The Quick Start guide (`docs/quick-start-guide/`) carries its own token copy and screenshots; it was **regenerated for Equus on 2026-09-22 (#166)** — its private `--gold` token is now `--green`, the cover uses the real reversed mark (`docs/quick-start-guide/logo-reverse.svg`), and all 18 screenshots come from staging. Regenerate with `capture.ts` then `render.ts` whenever the UI moves.
...
- **Local-only folders moved in-tree 2026-09-22, both gitignored — this repo is PUBLIC, so never `git add -f` either:**
  - **`_secrets/`** — plaintext credential notes (was `C:\Projects\_secrets-outside-repos\`). The password manager remains the canonical store; this is a convenience copy.
  - **`client-docs/`** — client-originated files: the 9.11 punchlist, KP July-2026 feedback, the recovered submissions JSON (was `C:\Projects\impact-client-docs\`).
  - `.gitignore` covers each by path, bare name and trailing slash, plus `**/*password*.md`. After touching `.gitignore`, re-verify: `git check-ignore -v _secrets client-docs`.
...
RR v7 + TypeScript 5.7 + Vite 6 + Vitest 3 + Playwright + ESLint 9 + Prettier 3 + Supabase Postgres (Auth + RLS) + **Drizzle ORM 0.45** + postgres-js + **@supabase/ssr 0.12** + Resend (wired in code; `RESEND_API_KEY` unset in every context, so `sendEmail` always fails into its try/catch) + Netlify. Dependencies were bumped as a group 2026-09-22 (#162: drizzle-orm 0.36→0.45, drizzle-kit 0.28→0.31, @supabase/ssr 0.5→0.12, Playwright, Sentry, tsx, prettier, typescript-eslint). Verified on staging before merge — including a real password reset, since the @supabase/ssr jump is the one thing CI cannot prove.
...
- **Tests**: `npm test` (Vitest unit, **381** today); `npm run test:rls` (RLS integration, requires explicit `BEGIN`/`COMMIT` to make `SET LOCAL ROLE authenticated` take effect against the BYPASSRLS connection); `npm run test:e2e` (Playwright, reads `.env.test`).
- **`supabase/config.toml`** registers the `custom_access_token_hook`; `supabase start` reads it too, so CI mirrors impact-dev.
- **Any action that calls a Supabase auth method and RETURNS DATA must return the `headers`.** `@supabase/ssr` runs the **PKCE** flow, so calls like `resetPasswordForEmail()` write a **code-verifier cookie** through the cookie adapter into the `Headers` you passed to `createSupabaseServerClient`. Return a bare object and that `Set-Cookie` is dropped, the browser never stores the verifier, and `exchangeCodeForSession()` in `/auth/callback` fails — presenting as the link silently bouncing to `/login`. Use `data(payload, { headers })`. Actions that finish with `throw redirect(..., { headers })` were always fine; it is only the data-returning branch that loses them. Broke password reset in every environment from SP5 until 2026-09-22 (#163). Regression test: `tests/routes/_public.auth.forgot.test.ts`.
- **PKCE also means a recovery/invite link only works in the browser that requested it.** `/login` surfaces this: `/auth/callback` redirects to `?error=link-invalid` on a failed exchange, and `loginNotice()` renders that plus `no-employer`, `employer-missing` and `reset=ok`. Those four codes were being *sent* by three routes and displayed by none, which is what made the reset failure so hard to read.
- **Dev-only features gate on `import.meta.env.DEV`, never `process.env.NODE_ENV`.** Netlify builds with `NODE_ENV=production` but serves from a Lambda where it is **unset**, so build-time and runtime disagree: `routes.ts` (build time) left `/dev/reseed` unregistered while the Settings → Program Info Danger Zone (runtime) rendered on staging and prod, a red destructive button whose target 404s. Vite substitutes `import.meta.env.DEV` at build time, so it agrees everywhere — the production build now tree-shakes the card out entirely. `routes.ts` itself keeps `process.env.NODE_ENV`: it is read by the Vite config, not bundled, so that is correct *there*. Guarded by `tests/guards/dev-only-gating.test.ts` (#165).
- **Supabase Auth redirect allow-list is a silent failure mode.** An unlisted `redirectTo` is **discarded, not rejected** — Supabase substitutes the project's **Site URL** and sends the mail anyway, with no error. impact-dev's list was empty until 2026-09-22, so every recovery and invite link pointed at the scaffold default `http://localhost:3000`. Both projects are configured now; wildcards (`https://host/**`) are less brittle than listing every `?next=` variant. If a link lands somewhere unexpected, check Authentication → URL Configuration **before** debugging code. Detail in `docs/deployment.md` §3.
...
The Supabase dashboard paste flow is documented in `docs/deployment.md`. `RESEND_API_KEY` is still unset in every Netlify context, so `sendEmail` is wrapped in `try/catch` with `console.warn` (non-fatal) — failed sends don't break the invite flow.

**What actually sends mail today (2026-09-22):** Supabase's built-in SMTP, not Resend. The builders in `app/emails/` are **paste sources** for the Supabase dashboard, deliberately not imported at runtime — `password-reset.tsx` having no importer is by design, not dead code. All four templates (Reset password + Invite user × impact-dev + impact-prod) were pasted and verified 2026-09-22.

**A pasted template freezes the program name.** `resolveProgramName()` (`invites.server.ts`) reads the `program_info` singleton so the *Resend* path follows Settings → Program Info, but the pasted copies are literal strings — rename the program and they must be re-rendered and re-pasted.

**The email masthead logo is a PNG, not the app's SVG** — Gmail and Outlook strip inline SVG and refuse `<img>` pointing at `.svg`. `public/email-logo.png` is generated from `logo-reverse.svg` by `npm run email:logo` and referenced by ABSOLUTE url (an email has no origin), so it only resolves once that build is on production.
